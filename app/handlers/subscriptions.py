...
from app.handlers.common import get_keyboard
from app.handlers.weather import get_weather
from app.scheduler.notifications import scheduler_notifications_start, scheduler_notifications_stop
...
async def show_weather(message: types.Message, state: FSMContext):
    ...
    weather = get_weather(coords=coords)
...
def get_inline_keyboard():
    buttons = [
        types.InlineKeyboardButton(text="\u2795", callback_data="time_incr"),
        types.InlineKeyboardButton(text="\u2796", callback_data="time_decr"),
...
def register_handlers_subs(dp: Dispatcher):
    ...
    dp.register_message_handler(cmd_subs, Text(endswith="Список отслеживаемых", ignore_case=True), state="*")
    dp.register_message_handler(edit_list_of_subs, Text(endswith="Редактировать"), state=ListOfSubscriptions.waiting_action)
    ...
    dp.register_message_handler(open_list_of_subs, Text(endswith="Открыть список"), state=ListOfSubscriptions.waiting_action)
